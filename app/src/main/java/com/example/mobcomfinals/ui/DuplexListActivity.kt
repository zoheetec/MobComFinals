...
class DuplexListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        authenticationViewModel.getStates().observe(this@DuplexListActivity) {
            handleState(it)
        }
        ...
        propertyViewModel.getRealtimeUpdate("Duplex")
        //----------------------------------------------------------------
    ...
    private fun handleState(state : AuthenticationStates) {
        when(state) {
            ...
            AuthenticationStates.LogOut -> {
                LoginActivity.launch(this@DuplexListActivity)
                finish()
            }
    ...
    companion object {
        fun launch(activity : Activity) = activity.startActivity(Intent(activity, DuplexListActivity::class.java))
    }
}